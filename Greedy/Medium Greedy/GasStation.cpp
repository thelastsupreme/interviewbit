...
int canCompleteCircuit(const vector<int> &A, const vector<int> &B) 
{
    ...
    /*
        For solution to even exist, out sum of gas (sum of A elements)
        must be >= to sum of cost (sum of B elements)
    */
    
    int sumOfGas = 0;
    int sumOfCost = 0;
    int startingGasStation = 0;
    //when amount of gas in tank becomes negative, we know new start must be picked
    int tank = 0;
    ...
    //there is a solution only if we have enough gas overall
    return sumOfGas >= sumOfCost ? startingGasStation : -1;
}
